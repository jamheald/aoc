...
for (i = 0; i < len; i++) {
    ...
    var cat = input[i].join('');

    task1 = task1 + String.prototype.concat(...new Set(cat)).length;
}
...
for (i = 0; i < len; i++) {
    ...
    var len2 = input[i].length;
    var cat = input[i].join('');

    for (j = 0; j < alph.length; j++) {
        task2 = task2 + (cat.split(alph[j]).length-1 == len2);
    }
}
...
for (i = 0; i < len; i++) {
    ...
    for (j = aStart.charCodeAt(0); j <= aEnd.charCodeAt(0); j++) {
        task2 = task2 + (cat.split(String.fromCharCode(j)).length-1 == len2);
    }
}
